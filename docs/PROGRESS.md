...
最終更新: 2026-06-09 (x3 スイープを real 初期姿勢中心に修正、ID3 初動症状の根本対策)
...
**真の原因 (2026-06-09 特定)**: SOFollower.configure() の挙動に起因する起動時 Goal_Position 不整合

- LeRobot 0.4.4 の `SOFollower.connect()` → `configure()` は:
  1. `Maximum_Acceleration = 254` (最大) をセット
  2. `P_Coefficient` を 32→16 に **下げる** ("avoid shakiness")
  3. `with torque_disabled()` を抜ける際に torque を **必ず ON** にする
  4. **Goal_Position は前 session の残骸値のまま** (Goal=Present の自動同期は無い)
- 結果: x3 起動と同時に「stale Goal → 現在 Present」を max_acceleration で詰めようとする
- ID3 は重力負荷 + 機械抵抗で初動が出にくい関節。P=16 の弱めゲインでは初動トルクが足りず、手で初動を与えると追従できる現象になる
- 他関節は重力負荷が小さい or 抵抗が少ないので初動が出る

**dynamic_tracking が問題無かった理由**: 起動時に明示的に `Goal_Position = Present_Position` を全 motor に書いてから torque ON していたため、初動 delta が 0 でスムーズに始まれた。

**修正** (commit 別途): x3_sim_to_real_mirror.py を更新
- (a) `make_robot("real")` 直後に `real.apply_targets(q_real_init)` で Goal=Present を再書込し凍結
- (b) sim の qpos を real 現在角に同期 (`sim.data.qpos[:6] = q_real_init; mj_forward`)
- (c) sin sweep を **real 初期角を中心** に振る (0 ではなく)
- これで dynamic_tracking と同じ「Goal=Present から始まる」初動パターンになり、ID3 でも手助け不要で動く (実機検証: `--max-step-deg 20` で初動 jam 回避するモードで動作確認済 → 本修正で max-step-deg=2 でも動くはず)

**LeRobot 側への将来 issue 提起候補**: `SOFollower.connect()` で Goal_Position も Present に自動同期した方が安全 (現状の設計だと毎回 stale Goal リスクがある)。今回は ws 側で吸収。
