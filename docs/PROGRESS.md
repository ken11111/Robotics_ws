...
| **ID3 (elbow_flex) 動作異常** | ⏳ 調査中 — スティクション疑い、動的追従テスト未実施 |
...
| 動的追従テスト | `tests/check_dynamic_tracking.py` | **未実施** — 次の再開時に実行する |
...
### 🔴 ID3 (elbow_flex) 動作異常 (調査中)

**症状**:
- sim → 実機 ミラー (`x3_sim_to_real_mirror.py`) で ID3 だけ追従が遅れる
- ログに `'elbow_flex': {'original goal_pos': -9.33, 'safe goal_pos': 8.33}` が大量に出る (LeRobot の `max_relative_target=2°` clamp が「target は -9°、present は +6° で固定」状態)
- 持ち上げて手で初動を与えると動き始める
- 静止状態で「少しだけ持ち上げて手を離すと落ちる」

**判明していること**:
- ✅ レジスタ設定 (Max_Torque, Torque_Limit, P/D/I gain) は他 5 個と同一 (`check_motors_health.py`)
- ✅ ID3 温度のみ +2〜3℃ (動作中の余分な仕事を示唆する弱い傍証)
- ✅ 静的保持テスト (`check_holding_load.py`) では Load=+28/1000、drift=+4 raw で **正常保持** — ただし測定姿勢が ID3 を負荷していなかった可能性 (Goal=1961 = キャリブ中央 +35 raw ≈ +3° しか曲げてない)

**未確認**:
- ⏳ 動的追従テスト (`check_dynamic_tracking.py`) を ID3 と ID2 で比較 → スティクション/トルク不足の定量化
- ⏳ 物理 swap テスト (ID3 ↔ ID5 のモーター本体を入れ替え) — 症状がどちらに移動するかで個体不良か機械干渉かを断定

**作業再開時の最初のアクション** (どちらかを選択):

A. 動的追従テスト 2 本 (推奨、リスク無し、20 分):
```bash
source /home/ken/Robotics_ws/.venv/bin/activate
python tests/check_dynamic_tracking.py --motor-id 3   # ID3 を診断
python tests/check_dynamic_tracking.py --motor-id 2   # ID2 を比較対照
```
結果の `stuck_samples` / `max abs load` / `max tracking error` を比較。
- ID3 だけ stuck > 10% → スティクション確定 → 機械干渉 (#1 マウントネジ / #2 ケーブル / #3 ホーン) の物理確認に進む
- ID3 だけ load 飽和 → トルク不足 → 個体交換要請
- ID2 も似た傾向 → 設定起因 (max_step が小さすぎ等)

B. 物理 swap テスト (約 30 分、再キャリブ込み):
```
電源 OFF → ID3 と ID5 のサーボ本体を物理位置入れ替え
→ lerobot-calibrate を再実行
→ tests/x3_sim_to_real_mirror.py --amplitude-deg 5 で挙動確認
```
